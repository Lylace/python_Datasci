...
#mnist 데이터 다운로드
(x_tran, y_tran),(x_test, y_test) = mnist.load_data()

mnist = input_data.read_data_sets("MNIST/", one_hot=True)

#손글씨를 픽셀화 한 결과를 저장할 심블릭 변수 선언
#손글씨 이미지는 28x28 따라서 총 784픽셀이 필요
x = tf.placeholder(tf.float32, [None, 784])   #784개의 벡터 생성
...
#각 원소y들 간 log계산하고 합한 후 평균 계산

train_step = tf.train.GradientDescentOptimizer(0.5).minimize(cross_entropy)
#딥러닝 학습 시 오차와 비용을 최소화 하기 위해 경사하강법 사용
